...
use super::{config::MobileState, models::CommandError, scanner, storage, sync};
...
pub fn list_employees(state: &MobileState, filter: &Value) -> Result<Value, CommandError> {
...
pub fn create_employee(state: &MobileState, draft: &Value) -> Result<Value, CommandError> {
...
pub fn generate_employee_tokens(state: &MobileState) -> Result<Value, CommandError> {
    let client_id = sync::ensure_client_id(state)?;
    let mut connection = storage::database(&state.data_dir)?;
    let ids = {
        let mut statement = connection
...
pub fn list_shifts(state: &MobileState) -> Result<Value, CommandError> {
    let connection = storage::database(&state.data_dir)?;
    let mut statement = connection
...
pub fn create_shift(state: &MobileState, draft: &Value) -> Result<Value, CommandError> {
...
pub fn update_shift(state: &MobileState, id: i64, draft: &Value) -> Result<Value, CommandError> {
    validate_shift(draft)?;
    let client_id = sync::ensure_client_id(state)?;
    let mut connection = storage::database(&state.data_dir)?;
    let transaction = connection
...
pub fn list_id_cards(state: &MobileState, filter: &Value) -> Result<Value, CommandError> {
    let connection = storage::database(&state.data_dir)?;
...
pub fn get_geofence_settings(state: &MobileState) -> Result<Value, CommandError> {
    let connection = storage::database(&state.data_dir)?;
    let mut statement = connection
        .prepare(
...
pub fn save_scan_security(state: &MobileState, payload: &Value) -> Result<Value, CommandError> {
    let client_id = sync::ensure_client_id(state)?;
...
pub fn get_app_display_name(state: &MobileState) -> Result<String, CommandError> {
    let connection = storage::database(&state.data_dir)?;
...
pub fn save_app_display_name(state: &MobileState, name: &str) -> Result<String, CommandError> {
    let client_id = sync::ensure_client_id(state)?;
    let mut connection = storage::database(&state.data_dir)?;
    let transaction = connection
        .transaction()
...
pub fn create_holiday(state: &MobileState, draft: &Value) -> Result<Value, CommandError> {
    let client_id = sync::ensure_client_id(state)?;
...
pub fn update_holiday(state: &MobileState, id: i64, draft: &Value) -> Result<Value, CommandError> {
    let client_id = sync::ensure_client_id(state)?;
    let mut connection = storage::database(&state.data_dir)?;
    let transaction = connection
        .transaction()
...
pub fn delete_holiday(state: &MobileState, id: i64) -> Result<Value, CommandError> {
    let client_id = sync::ensure_client_id(state)?;
    let mut connection = storage::database(&state.data_dir)?;
    let transaction = connection
        .transaction()
...
pub fn list_holiday_whitelist(state: &MobileState) -> Result<Value, CommandError> {
    let connection = storage::database(&state.data_dir)?;
    let mut statement = connection
        .prepare(
            "SELECT id, scope_type, scope_value, tanggal_libur, keterangan, status_aktif
...
pub fn delete_holiday_whitelist(state: &MobileState, id: &str) -> Result<Value, CommandError> {
    let client_id = sync::ensure_client_id(state)?;
    let mut connection = storage::database(&state.data_dir)?;
    let transaction = connection
        .transaction()
...
pub fn get_alfa_settings(state: &MobileState) -> Result<Value, CommandError> {
    let connection = storage::database(&state.data_dir)?;
...
pub fn save_alfa_settings(state: &MobileState, enabled: bool) -> Result<Value, CommandError> {
    let client_id = sync::ensure_client_id(state)?;
...
pub fn get_company_profile(state: &MobileState) -> Result<Value, CommandError> {
    let connection = storage::database(&state.data_dir)?;
