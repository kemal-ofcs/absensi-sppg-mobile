...
fn session_token(state: &MobileState) -> String {
    state
        .session
        .lock()
        .ok()
...
#[tauri::command]
pub fn desktop_get_scan_security(state: State<'_, MobileState>) -> Result<Value, CommandError> {
...
pub fn apply_window_title(app: &AppHandle, state: &MobileState) {
