...
impl TursoClient {
    ...
    pub async fn ensure_schema(&self) -> Result<(), CommandError> {
        ...
        let default_elements_str =
            serde_json::to_string(&crate::mobile::operational::default_id_card_elements())
    ...
    async fn ensure_schema_current(&self) -> Result<(), CommandError> {
        let current = self
            .query_one(
                "SELECT COUNT(*) AS total FROM schema_migration WHERE version = -2003;",
                vec![],
            )
            .await
            .ok()
    ...
    pub async fn pull_snapshot(&self, last_revision: i64) -> Result<Value, CommandError> {
        ...
        let tables = [
            ...
            ("taxRules", "SELECT * FROM tax_rules ORDER BY category, bracket_min;"),
            ("bpjsRules", "SELECT * FROM bpjs_rules ORDER BY component_code;"),
            (
                "payrollRuns",
