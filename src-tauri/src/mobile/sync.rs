...
/// perangkat lain — dan `MobileState::load` membaca `turso_database_url` saat
...
pub fn apply_snapshot(state: &MobileState, payload: &Value) -> Result<usize, CommandError> {
...
fn ensure_unsynced_payroll_data_enqueued(state: &MobileState) -> Result<(), CommandError> {
...
mod tests {
    ...
    use super::{
        ...
        pending_events, storage, MobileState, CLIENT_SCHEMA_VERSION,
    };
